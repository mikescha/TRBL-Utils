...
OUT_ARI_FILE = Path("nestling-to-female-ratios-for-comparison.csv")
OUT_FILTERED_FILE = Path("nestling-to-female-ratios-filtered.csv")
...
def make_ratios(target_pmj_dir = PMJ_DIR, 
                output_file = OUT_FILE,
                filtered_output_file = OUT_FILTERED_FILE,
                ari_file = OUT_ARI_FILE,): 
    output_csv = DATA_DIR / output_file
    sharing_output_csv = SHARING_OUTPUT_DIR / output_file
    filtered_output_csv = DATA_DIR / filtered_output_file
    ari_output_csv = DATA_DIR / ari_file
    ...
    for _, row in breeding_dates_df.iterrows():
        ...
        female_path = target_pmj_dir / name / f"{name} Female.csv"
        nestling_path = target_pmj_dir / name / f"{name} Nestling.csv"
        fledgling_path = target_pmj_dir / name / f"{name} Fledgling.csv"
    ...
    if results:
        ...
        # ------------------------------------------------------------------
        # TODO: If there are fledglings, then we're calling it either Paritially Abandoned or Successful despite the ARI score. 
        ...
        save_csv_with_retry(for_comp, ari_output_csv)
